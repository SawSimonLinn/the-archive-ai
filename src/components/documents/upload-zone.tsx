  ...
  return (
    <div className="space-y-6">
      <div
        {...getRootProps()}
        className={cn(
          "border-4 border-dashed rounded-none p-16 flex flex-col items-center justify-center transition-all relative",
      ...
      >
        <div className="absolute top-4 left-4 flex items-center gap-2 font-mono text-[8px] font-black uppercase tracking-[0.3em] opacity-40">
          <Fingerprint className="h-3 w-3" /> Secure Upload
        </div>
        <input {...getInputProps()} />
        ...
        <h3 className="font-headline font-black text-2xl uppercase tracking-tighter text-center">Upload Documents</h3>
        <p className="text-sm font-bold text-muted-foreground mt-2 text-center max-w-sm mb-8 uppercase tracking-tight">
          Drag and drop PDF, TXT or DOCX files here.
        </p>
        <Button 
          variant="outline" 
          type="button" 
          onClick={open}
          className="h-12 px-8 border-2 border-foreground rounded-none font-black uppercase tracking-tighter hover:bg-foreground hover:text-background transition-all"
        >
          Select Files
        </Button>
      </div>
              ...
              <div key={file.name} className="flex items-center justify-between p-4 bg-card border-2 border-foreground shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="font-mono text-[9px] font-bold uppercase tracking-widest opacity-40">{(file.size / 1024 / 1024).toFixed(2)} MB // Ready to upload</span>
                  </div>
                </div>
                {!uploading && (
